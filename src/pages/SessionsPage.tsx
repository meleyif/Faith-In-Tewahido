...
import { Video, Clock, Users, Link as LinkIcon, Calendar, Loader2 } from "lucide-react";
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    fetch('/api/v1/sessions/upcoming')
      .then(res => res.json())
      .then(data => {
  ...
  const handleJoin = (e: React.MouseEvent) => {
    e.preventDefault();
    alert("In a real environment, this would launch the Zoom/Meet integration or redirect to the live video classroom.");
  };
  ...
  return (
    <div className="space-y-12 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="text-center font-mono text-muted py-20 bg-layer1 border border-gold-border border-dashed">
          No upcoming sessions found for your enrolled courses.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sessions.map((session, i) => (
            <div key={session.id} className="bg-layer1 border border-gold-border p-6 flex flex-col group relative">
              ...
              <button 
                onClick={i === 0 ? handleJoin : undefined}
